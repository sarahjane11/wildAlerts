...
class AccountsettingController extends UsersController
{
	/**
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function actionChangePassword(){
            ...
            if(isset($_POST['User']))
            {
                ...
                }else if(trim($_POST['User']['newpassword'])==''){
                    Yii::app()->user->setFlash('error2', "New password cannot be blank or whitespace.");
     
                
                }else{
                    //echo '3';die;
                    //echo 'whitespace'.$_POST['User']['newpassword']; die;
                    $user = User::model()->findByPk($id);
                    
                        if($user->password != md5($_POST['User']['oldpassword'])){
                            Yii::app()->user->setFlash('error3', "Old password is incorrect.");

                       }
                       else{
                           $user->password = md5(trim($_POST['User']['newpassword']));
                           $user->update();
        ...
        public function actionProfile(){
             ...
             $getlocation = UserLocations::model()->findAllByAttributes(array('user_id'=>$id));
              
            
            $this->render('profile',array(
			'model'=>$model,'getlocation'=>$getlocation
		));
        }
}
